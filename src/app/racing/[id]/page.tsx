...
import { Clock, MapPin, Trophy, ArrowLeft, Info } from "lucide-react"
...
export async function generateStaticParams() {
    ...
    return events
        .filter(event => event.type === 'racing')
        .map((event) => ({
            id: event.id,
        }))
}
...
export default async function RacingEventPage({ params }: PageProps) {
    const { id } = await params
    const event = await getEventById(id)

    if (!event || event.type !== "racing") {
        notFound()
    }
    ...
    return (
        <div className="container mx-auto px-4 md:px-6 py-8">
            <Link href="/racing" className="inline-flex items-center text-sm text-zinc-500 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-zinc-50 mb-6">
                <ArrowLeft className="h-4 w-4 mr-1" /> Back to Racing
            </Link>
            ...
            <div className="grid gap-6 lg:grid-cols-3">
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 p-6">
                        ...
                        <div className="bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-900/50 rounded-lg p-4 mb-6">
                            ...
                            {event.competitors.map((comp) => (
                                <div key={comp.id} className={`flex items-center justify-between p-3 rounded-lg border ${comp.id === winner?.id ? 'border-green-200 bg-green-50 dark:bg-green-900/10 dark:border-green-900/50' : 'border-zinc-100 dark:border-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-800/50'}`}>
                                    <div className="flex items-center gap-3">
                                        {comp.number && (
                                            <span className="flex h-6 w-6 items-center justify-center rounded bg-zinc-200 text-xs font-bold text-zinc-700 dark:bg-zinc-700 dark:text-zinc-300">
                                                {comp.number}
                                            </span>
                                        )}
                                        <div>
                                            <div className="font-medium text-sm">{comp.name}</div>
                                            {comp.jockey && <div className="text-xs text-zinc-500">{comp.jockey} (Bar: {comp.barrier})</div>}
                                        </div>
                                    </div>
